...
| 底部按钮组 | `themes/arknights/layout/includes/bottom-btn.pug` + `source/css/_page/post/bottom_btn.styl` | 右列单列 flex 栈；返回上一页 / 工具箱（独立锚定左下角、与切换主题底缘齐平；扇形展开 `.toolbox-open`、层叠容量 3、悬停抽出）；标注 `hl-mark` / 分享 `.copied` / 收藏 `.saved` 视觉态；JS 契约 `toolbox.*` 实现于 `Toolbox.ts`（标注 / 收藏持久化 `arknights:*`，载入 + pjax 恢复） |
