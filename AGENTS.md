...
- **A2 工具箱静态基线**：已完成工具箱 toggle 与标注 / 分享 / 收藏 / 截图 / BGM 五项的静态 DOM/CSS 契约；截图与 BGM 的行为控制器、交互接线及相关配置验收仍待 C 阶段，A2 不代表这些交互功能已完成。
...
- **项目列表页面**：`source/projects/index.md` 使用严格 `[#]<PJ>{"名称", "链接", "图片路径"}`；`handlers/projects.js` 分别按 URL、HTML 文本/属性和 CSS URL 上下文安全序列化，保留 `--card-img`、懒加载图片、`.project-name`、`target="_blank"`、`rel="noopener"` 及 `source/js/project-tooltip.js` 所需结构与 Pjax 重绑契约
...
| 项目卡片交互 | `source/projects/index.md` + `markers/handlers/projects.js` + `source/js/project-tooltip.js` | 保留 `.projects-grid > .project-card`、`--card-img`、懒加载图片、`.project-name`、安全 URL/CSS、悬停和 Pjax 重绑；本轮 marker 迁移不修改 CSS、TypeScript 或该脚本，若未来修改相应产物须递增缓存版本 |
...
- 最终状态运行 `node --check .temp/marker-e2e.test.js`、`node --check .temp/marker-artifacts.js`、`git diff --check`、`git diff --stat`、`git status --short`，随后只运行一次 `TZ=Asia/Shanghai npm run build` 并立即运行 `node .temp/marker-artifacts.js`；artifact 探针检查三篇 AI 文章、项目页、`public/search.json` 与 `public/js/project-tooltip.js`
