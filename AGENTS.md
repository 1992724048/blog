...
- **缓存版本号**：当前主题产物版本为 `cssVersion=20260952`、`jsVersion=20260950`；修改主题 CSS 产物（`arknights.css`，即主题 Stylus 源）后递增 `themes/arknights/layout/includes/meta-data.pug` 中的 `cssVersion`，修改主题 JS 产物（`arknights.js` / `search.js`）后递增 `themes/arknights/layout/includes/js-data.pug` 中的 `jsVersion`——均用于避免 Cloudflare / 浏览器缓存旧版
...
- 最终状态运行 `node --check .temp/marker-e2e.test.js`、`node --check .temp/marker-artifacts.js`、`git diff --check`、`git diff --stat`、`git status --short`；随后在同一最终状态只执行一次 PowerShell 顺序：`npm run clean` → `$env:TZ = 'Asia/Shanghai'; npm run build` → `node .temp/marker-artifacts.js` → `node .temp/http-smoke.js` → `node .temp/nav-smoke.js` → `node .temp/r10-toolbox-geometry.js`。artifact 探针检查三篇 AI 文章的键盘 tooltip 唯一性、项目 / 数据 / 文章评论、Pjax 区外唯一 audio、工具箱五项、`public/search.json` 的 `STATE + 单空格 + text` 且无 tooltip/四态说明/加密明文/内部串、SnapDOM vendor hash、CSS/JS 版本与 `public/js/arknights.js` 中的 ProjectTooltip 合并契约
- AI tooltip、项目悬停、工具箱五项、真实截图 PNG、BGM 播放 / 错误重试、导航 / footer 断点、懒加载、搜索与站内 Pjax 重绑仍须真实有头浏览器人工验收；无头截图不能作为通过证据，未执行人工门禁时不得声称浏览器验收完成
- 临时脚本 / 产物放 `.temp/`（已 gitignore），用完清理；`node .temp/nav-smoke.js` 为顶栏 / 搜索 / 工具箱交互的 jsdom 冒烟脚本（读取代表性文章 `public/`），`node .temp/r10-toolbox-geometry.js` 校验最终五项 CSS 几何、状态节点与文章 / 非文章 DOM 分支；两者按上一条最终构建顺序在 Hexo build 后运行
