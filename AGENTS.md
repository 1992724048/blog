...
- **AI/PJ carrier/Marked 迁移（已批准、实施中）**：新架构计划使用每次 `post.render` 私有 carrier、`data.markdown` 非枚举 options 与可枚举模块私有 `CARRIER_SYMBOL` bridge；`marked:use` priority 0、before 4、after 9，extension 的 start/tokenizer 读取 `this.lexer.options`，`processAllTokens` 读取 `this.options`，renderer 通过 `this.parser` 委托，`walkTokens` 只审计 token metadata。raw 保护只针对实际 block/raw-text `html` token（block HTML、`pre`/`textarea`/`script`/`style`），普通 inline HTML 标签间文本仍可物化；image alt、link label、link URL/title、HTML 属性不接收 HTML wrapper。当前 runtime 仍保留旧 AI/PJ 路径，`carrier.js`/`marked-extension.js` 尚未完成，旧路径与旧标记只能由 Task 5 在迁移验证后删除。
- **carrier 生命周期边界**：renderer 拒绝时 Hexo 不会执行 after filter；实现不得依赖 after-finally 清理，也不得保留全局 carrier 强引用。下一次同 data before 必须修复/清理旧字段和 `data.markdown` descriptor；`data.more` 永不作为 marker 输入，Hexo excerpt priority 10 从已物化 content 派生。`dompurify: false` 或未配置时的 identity sanitizer 是 bridge 的明确支持边界，其他 sanitizer 配置必须由真实探针证明不会改写 wrapper。
...
| 待实施 markers 模块 | `themes/arknights/scripts/markers/` | Task 1–3 的 lexer/parser/token/registry/AI/PJ handler 已有基线；Task 4 待完成 `carrier.js`、`marked-extension.js`、精确 sentinel/pipeline/唯一注册入口并替换旧 raw 来源判断；Task 5 迁移内容后删除旧路径 |
...
themes/arknights/scripts/markers/ # Task 1–3 基线 + 旧 Task 4 runtime；carrier/Marked bridge 为 Task 4 待实施文件
...
- 标记解释器 Task 4 的实施门禁应覆盖真实 Hexo 自动注册（before 4、after 9、`marked:use` 0）、实际 block/raw-text `html` token 与普通 inline HTML 对照、image alt/link label/link URL-title/HTML 属性无 wrapper、非文本上下文原文恢复、两个连续 heading-only 与 `_headingId` 不污染、renderer rejection 后同 data 重试清理、显式 excerpt 与 Hexo 派生 more，以及 `dompurify: false` 边界；本轮文档任务不执行这些运行时代码探针。
...
- 标记解释器迁移的全文搜索按范围分层：运行时代码/内容检查 `source/` 与 `themes/arknights/scripts/`，文档契约检查本文件与 `docs/2026-09-24-marker-interpreter-*.md`；全仓审计排除 `node_modules/`、`.git/`、`.temp/`、`public/` 与 `docs/superpowers/`，不能把迁移表中的历史 `[&]` 示例误报为运行路径。
- 标记解释器协议固定为严格 `[#]<NAME>{...}` 参数、handler 整枚原子失败、raw block/raw-text 与 inline Markdown text 分离；旧 `[&]` 语法和旧 AI/PJ 路径只在 Task 5 迁移验证后删除。
