...
- **AI/PJ carrier/Marked 迁移（已批准、实施中）**：新架构公开 store-owned `findOccurrences(value, field)` / `bindContext(id, context)`、深度冻结 parent occurrence metadata 与状态迁移；扩展不得拆 token。before 4 为每次 `post.render` 创建私有 carrier，并通过非枚举 `data.markdown` options 与可枚举模块私有 `CARRIER_SYMBOL` bridge 传递。`marked:use` priority 0：start/tokenizer 读取 `this.lexer.options`（Marked 15 实际传 `src.slice(1)`，start 返回值不得再加一）；`processAllTokens` 读取 `this.options`，按 child-first canonical ownership 处理 `image.text → image-alt`、`link.tokens/text → link-label`、`link.href → link-url`、仅字符串 `link.title → link-title`、实际 `html` block → raw，并只审计 `field==='content'` 的 Marked occurrence；具体 child field 优先，严格祖先只复制已确定 snapshot，不兼容冲突抛 `CARRIER_BINDING_ERROR`。metadata descriptor 固定为不可枚举/不可写/不可配置，数组/元素/嵌套 parent 深度冻结；不扫描 `link.raw`。renderer 只消费 metadata 并通过 `this.parser` 委托，`walkTokens` 只做 token-local 检查。lexer 将 HTML 标签及属性作为 protected 原文，属性 marker 不签发 occurrence/metadata；Task 4 增量完整保护 `pre`/`textarea`/`script`/`style` raw-text。已签发 raw occurrence 只由 `token.type==='html' && token.block===true` 恢复；普通 inline `<span>/<a>/<em>` 间文本仍可物化。link label 是 text-carrier；image alt 唯一采用当前 Hexo `image.text`，link href 与字符串 title raw-preserve，均不调用 handler；最终 link/image 验收按 `<a>`/`<img>` renderer DOM 语义，不要求 Markdown 原文。heading-only 把原 `token.tokens` 同一引用传给 `parseInline`；该矩阵只在真实 `Hexo#post.render` 验收，纯 `new Marked()` 只测扩展基本行为。当前 runtime 仍停在旧 Task 4 baseline，`carrier.js`/`marked-extension.js` 尚未实现，旧 AI/PJ 路径与旧标记只能由 Task 5 在迁移验证后删除。
- **新架构 carrier 生命周期边界（待 Task 4 实施）**：应用不建立 global current carrier，但 Marked 15 singleton 可能在当前/下一次 parse 前短暂强引用 parse options；`processAllTokens` 在 finally 从当前 parse options 副本删除 `CARRIER_SYMBOL`，只把深度冻结审计快照附到 token，原始 `data.markdown` bridge 仍由 after/同 data before 恢复。Hexo 顺序为 renderer → `onRenderEnd` → `after_render:html` → after 9；任一前序阶段拒绝时 Hexo 不执行 after filter，下一次同 data before 必须修复字段和 descriptor。`getCarrierFromOptions(options, expectedCarrier)` 必须做 expected carrier 同一引用校验。显式 excerpt 不经过 Marked，由 after 9 审计 `excerpt-pending`；`data.more` 永不作为 marker 输入或由 pipeline 读写，并用抛异常 getter/setter spy 证明，Hexo excerpt priority 10 从已物化 content 派生。无显式 excerpt 时 `projectText(data,'excerpt')` 从已保存 content projection 按 `<!-- more -->` 派生，无分隔符回退 content。字段级 fallback 以 `carrier.originalField(field)` 为权威，`store.restore` 仅局部 best effort。Task 4 只测试 projection；`data.description` 与 `meta-description.js` 接线/断言归 Task 5。`dompurify: false` 或未配置时的 identity sanitizer 是 bridge 支持边界，其他配置必须由真实探针证明不会改写 wrapper。
...
- 标记解释器 Task 4 的实施门禁应覆盖真实 Hexo 自动注册与 priority（before 4、after 9、`marked:use` 0）、renderer/`onRenderEnd`/`after_render:html` 边界、`x <pre>/<textarea>/<script>/<style>` 完整保护、block raw/空行 paragraph/Markdown 生成块/inline HTML 对照、store/carrier spy 证明 protected 区域无 occurrence/metadata 且正常 content 真实 token 有深度冻结 metadata、child-first link/image canonical ownership 与稳定 `CARRIER_BINDING_ERROR`、当前 Hexo `image.text` alt、link label/href/字符串 title/autolink/裸 URL 与同段普通 marker 共存（按 `<a>/<img>` DOM 语义验收）、`processAllTokens` 只审计 content 与 after 9 审计 excerpt-pending、抛异常 `data.more` getter/setter、`carrier.originalField(field)` fallback、真实 `Hexo#post.render` 中两个连续 heading-only/失败/PJ/`headerIds:false`/同一 children 引用/`_headingId`、两个连续 PJ 的单网格/双卡片/无额外 p wrapper及普通文字/空行中断、Marked options symbol 清理、显式/派生/无分隔符 projection、`dompurify: false`/identity 边界。纯 `new Marked()` 只测扩展基本行为；Task 4 不测试 `data.description` 或 meta-description filter，相关断言归 Task 5；本轮文档任务未执行这些 runtime gate。
