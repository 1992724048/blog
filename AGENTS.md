...
- 搜索投影最终门禁运行 `node .temp/search-projection-lifecycle.test.js`：必须跨真实 Warehouse `db.json` 加载、locals clone 与缓存构建，覆盖已有非 null content、一次性自愈、损坏 sidecar、path 删除/复用、Terms、普通全文、frontmatter/tag/origin/模糊加密 fail-closed，并证明不会读取加密 `origin`/`_content` 或回退旧 HTML
