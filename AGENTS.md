...
- **AI/PJ carrier/Marked 迁移（已批准、实施中）**：新架构使用 store-owned `findOccurrences(value, field)` / `bindContext(id, context)` 管理 occurrence id/context/state；before 4 为每次 `post.render` 创建私有 carrier，并通过非枚举 `data.markdown` options 与可枚举模块私有 `CARRIER_SYMBOL` bridge 传递。`marked:use` priority 0：start/tokenizer 读取 `this.lexer.options`（Marked 15 实际传 `src.slice(1)`，start 返回值不得再加一），`processAllTokens` 读取 `this.options`，独占实际 token 树上的发现/重分类/字段恢复与冻结 `parent.arknights` metadata 数组，renderer 只消费 metadata 并通过 `this.parser` 委托，`walkTokens` 只审计 token。lexer 完整保护 `pre`/`textarea`/`script`/`style` raw-text；已签发 raw occurrence 只由实际 block `html` token 恢复，普通 inline `<span>/<a>/<em>` 间文本仍可物化；image alt、link label、link URL/title、HTML 属性不接收 wrapper。heading-only 保留原 children 供 after 9 物化/恢复，不进入 Hexo `_headingId` 路径。当前 runtime 仍停在旧 Task 4 baseline，`carrier.js`/`marked-extension.js` 尚未实现，旧 AI/PJ 路径与旧标记只能由 Task 5 在迁移验证后删除。
- **新架构 carrier 生命周期边界（待 Task 4 实施）**：应用不建立 global current carrier，但 Marked 15 singleton 可能在当前/下一次 parse 前短暂强引用 parse options；`processAllTokens` 在 finally 从当前 parse options 副本删除 `CARRIER_SYMBOL`，只把冻结审计快照附到 token，原始 `data.markdown` bridge 仍由 after/同 data before 恢复。renderer 拒绝时 Hexo 不执行 after filter；下一次同 data before 必须修复字段和 descriptor。`data.more` 永不作为 marker 输入或由 pipeline 读写，Hexo excerpt priority 10 从已物化 content 派生；无显式 excerpt 时 `projectText(data,'excerpt')` 只从已保存 content projection 按 `<!-- more -->` 派生。`dompurify: false` 或未配置时的 identity sanitizer 是 bridge 支持边界，其他配置必须由真实探针证明不会改写 wrapper。
...
- 标记解释器 Task 4 的实施门禁应覆盖真实 Hexo 自动注册（before 4、after 9、`marked:use` 0）、`x <pre>/<textarea>/<script>/<style>` 完整保护与源 block raw `<ol>/<blockquote>/<table>` / Markdown list-quote-table / inline `<span>/<a>/<em>` 对照、store occurrence API 与 parent `arknights` 多 occurrence、image alt/link label/实际含 marker 的 link URL-title/HTML 属性无 wrapper、两个连续 heading-only 保留原 children 并在 after 9 物化/恢复且不产生空/`-1` `_headingId` 计数、renderer rejection 后同 data 重试与 Marked options symbol 清理、显式/派生 excerpt 和 `PASS 摘要说明` meta description，以及 `dompurify: false`/identity 边界；本轮文档任务不执行这些运行时代码探针。
