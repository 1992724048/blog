...
- **搜索投影跨 Warehouse 身份边界**：marker pipeline 在原 post 的 `after 9` 阶段按稳定 `path` 保存纯文本投影；Warehouse locals clone 只通过 `projectTextByPath` 读取，不对 clone 调 `projectText`，无投影时回退到已保存的渲染内容
