...
| 定制脚本 | `themes/arknights/scripts/`（filters / tags / generator） | 术语自动链接、文章加密、搜索数据、build_time、minify、`??内容??` 遮盖（`filters/spoiler.js`，悬停显示）、AI 生成内容标记（`filters/ai-badge.js` + `ai-badge-core.js`，`[&]AI|PASS|文本|` → 四态徽标 PASS/EDIT/IGNORE/NOTREVIEW，样式在 `custom.styl` `.ai-badge`）、meta description 剥徽标（`filters/meta-description.js`，优先级 20：剥离已渲染 `.ai-badge` 与原始 `[&]AI|…|` 标记，无 frontmatter 时从 excerpt/content 派生 ≤160 字纯文本）等 |
