...
- **AI/PJ carrier/Marked 迁移（已批准、实施中）**：新架构公开 store-owned `findOccurrences(value, field)` / `bindContext(id, context)`、冻结 parent occurrence metadata 数组与状态迁移；扩展不得拆 token。before 4 为每次 `post.render` 创建私有 carrier，并通过非枚举 `data.markdown` options 与可枚举模块私有 `CARRIER_SYMBOL` bridge 传递。`marked:use` priority 0：start/tokenizer 读取 `this.lexer.options`（Marked 15 实际传 `src.slice(1)`，start 返回值不得再加一）；`processAllTokens` 读取 `this.options`，按 `image.text → image-alt`、`link.tokens/text → link-label`、`link.href → link-url`、非 null `link.title → link-title`、实际 `html` block → raw 的固定顺序独占发现/重分类/完整 occurrence 审计/字段恢复/metadata；不扫描 `link.raw`。renderer 只消费 metadata 并通过 `this.parser` 委托，`walkTokens` 只做 token-local 结构检查。lexer 将 HTML 标签及属性作为 protected 原文，属性 marker 不签发 occurrence/metadata；Task 4 增量完整保护 `pre`/`textarea`/`script`/`style` raw-text。已签发 raw occurrence 只由 `token.type==='html' && token.block===true` 恢复；普通 inline `<span>/<a>/<em>` 间文本仍可物化。image alt 与 link href/title 只 raw-preserve，只有 link label 是 text-carrier（状态 `text-preserved`，可委托 link renderer），三者均不调用 handler；image `text`/`tokens` 走同一安全 adapter。heading-only 把原 `token.tokens` 同一引用传给 `parseInline`，供 after 9 物化/恢复且不进入 Hexo `_headingId` 路径。当前 runtime 仍停在旧 Task 4 baseline，`carrier.js`/`marked-extension.js` 尚未实现，旧 AI/PJ 路径与旧标记只能由 Task 5 在迁移验证后删除。
- **新架构 carrier 生命周期边界（待 Task 4 实施）**：应用不建立 global current carrier，但 Marked 15 singleton 可能在当前/下一次 parse 前短暂强引用 parse options；`processAllTokens` 在 finally 从当前 parse options 副本删除 `CARRIER_SYMBOL`，只把冻结审计快照附到 token，原始 `data.markdown` bridge 仍由 after/同 data before 恢复。renderer 拒绝时 Hexo 不执行 after filter；下一次同 data before 必须修复字段和 descriptor。`data.more` 永不作为 marker 输入或由 pipeline 读写，Hexo excerpt priority 10 从已物化 content 派生；无显式 excerpt 时 `projectText(data,'excerpt')` 从已保存 content projection 按 `<!-- more -->` 派生，无分隔符回退 content。Task 4 只测试 projection；`data.description` 与 `meta-description.js` 接线/断言归 Task 5。`dompurify: false` 或未配置时的 identity sanitizer 是 bridge 支持边界，其他配置必须由真实探针证明不会改写 wrapper。
...
- 标记解释器 Task 4 的实施门禁应覆盖真实 Hexo 自动注册与 priority（before 4、after 9、`marked:use` 0）、`x <pre>/<textarea>/<script>/<style>` 完整保护、无空行源 block raw `<ol>/<blockquote>/<table>` / 空行拆分 paragraph / Markdown list-quote-table / inline `<span>/<a>/<em>` 对照、公开 occurrence/context API 与冻结 parent metadata 多 occurrence、image `text`/`tokens` 同一安全 adapter、link label/href/title/null title、禁止扫描 `link.raw`、HTML 属性原样且无 occurrence/metadata、两个连续 heading-only 的 token spy 确认 `parseInline` 收到原 children 同一引用并在 after 9 物化/恢复、`headerIds:false`、无空/`-1` `_headingId`、renderer rejection 后同 data 重试与 Marked options symbol 清理、显式/派生/无分隔符回退的 `projectText` 投影，以及 `dompurify: false`/identity 边界。Task 4 不测试 `data.description` 或 meta-description filter，相关断言归 Task 5；本轮文档任务不执行这些运行时代码探针。
