...
- 最终状态运行 `node --check .temp/marker-e2e.test.js`、`node --check .temp/marker-artifacts.js`、`git diff --check`、`git diff --stat`、`git status --short`，随后只运行一次 `TZ=Asia/Shanghai npm run build` 并立即运行 `node .temp/marker-artifacts.js`；artifact 探针检查三篇 AI 文章、项目 / 数据 / 文章评论、Pjax 区外唯一 audio、工具箱五项、`public/search.json` 的 `STATE + 单空格 + text` 且无 tooltip/四态说明/加密明文/内部串、SnapDOM vendor hash、CSS/JS 版本与 `public/js/arknights.js` 中的 ProjectTooltip 合并契约
