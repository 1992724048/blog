...
- **底部按钮组**：`bottom-btn.pug` + `source/css/_page/post/bottom_btn.styl`（随 article 交换、各页面显示）；`.bottom-btn-stack` 单列 flex 栈（DOM 顺序即视觉顺序）——依次为回到顶部 `#to-top` / 返回上一页 `#to-back`（`history.back()`）/ 目录 `#to-index`（≥769px 隐藏）/ 切换主题 `#color-mode` / 工具箱 `.toolbox`（`bgm` 配置关闭时其按钮不渲染）；工具箱展开态类 **`.toolbox-open`**（挂载 `.toolbox` 根）——点击扇形展开（半径 58px、角度 180°/157.5°/135° 向左上；扇形容量 3，第 4 个起沿原点层叠成堆），≥769px 悬停沿径向外抽（位移 + 放大 + 投影）；工具为标注 / 分享 / 收藏（视觉态 `.toolbox-share.copied`、`.toolbox-favorite.saved`；收藏标题经 `data-label-default` / `data-label-saved` 切换；正文高亮元素类 `.hl-mark`）；接线契约 `toolbox.toggle()/annotate()/share()/favorite()`，TS 侧按 `Toolbox.ts` 预写（后续任务实现，当前仅 DOM 预留）
...
| 底部按钮组 | `themes/arknights/layout/includes/bottom-btn.pug` + `source/css/_page/post/bottom_btn.styl` | 单列 flex 栈；返回上一页 / 工具箱（扇形展开 `.toolbox-open`、层叠容量 3、悬停抽出）；标注 `hl-mark` / 分享 `.copied` / 收藏 `.saved` 视觉态；JS 契约 `toolbox.*` 由 `Toolbox.ts` 后续实现（当前仅 DOM 预留） |
