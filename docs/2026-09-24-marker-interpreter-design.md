...
- 文档状态：carrier provenance 架构修订已批准；本轮针对第三轮文档审查收敛生命周期、字段接口、heading-only、派生 excerpt 与 raw/inline 契约。本轮仅修订规格、实施计划与 `AGENTS.md`，并写入架构复核报告，不修改运行时代码、测试探针、内容或配置。
...
已批准的决定是改用 **临时 render carrier + 实际 Marked token provenance**。源码中的 raw occurrence 可以短暂携带内部 carrier；`processAllTokens` 负责在实际 token 树上发现、重分类并附上冻结审计快照，renderer 只消费已准备 metadata。lexer 完整保护 `pre`/`textarea`/`script`/`style` raw-text；只有实际 block `html` token 的 provenance 才能恢复已签发 raw occurrence。普通 Markdown text 子树和普通 inline HTML 标签间文本保持 pending。carrier 不传给 handler、不进入 heading slug；priority 9 完成业务消费，正常 after 恢复字段 bridge。
...
3. 跳过代码与 HTML 属性；lexer 对 `pre`/`textarea`/`script`/`style` 保护从开始标签到匹配结束标签的完整 raw-text 内容。普通 block raw HTML 的 marker 仍可签发 token，但只能由当前 render 的实际 block `html` token provenance 恢复；普通 inline HTML 标签及其间文本仍按 Markdown text 语义处理，普通 Markdown 生成的 `ul`、`ol`、`blockquote`、`table` 仍必须正常解释其受支持 marker。
...
- 跳过 fenced code、缩进代码、行内代码和 HTML 属性。`pre`、`textarea`、`script`、`style` 必须保护完整开始标签、raw-text 内容与匹配结束标签；未闭合时保护到 EOF。该规则对行首 block 和 `x <pre>…` 等 inline 位置都成立。
- 普通 block-level raw HTML 只保护开始/结束标签及属性，不在 lexer 中把整个元素猜成 raw；元素内部 marker 可以先签发 token，之后只能由本次实际 block `html` token 的 provenance 恢复。普通 inline HTML 标签及其间文本保持 Markdown text 语义，例如 `<span>`、`<a>`、`<em>` 之间的 marker 可以物化。
- 标签属性是独立的 `html-attribute` raw-preserve 上下文；image alt、link label、link URL/title 另有专门上下文规则，不能把 carrier 或 HTML wrapper 放入这些字段。
...
- 为每个已提取候选生成 opaque token，并保存原始文本、block/inline 模式和随机校验值；token record 本身仍不写入 field、occurrence id、context 或 state。
...
- store 拥有 occurrence 注册表。`findOccurrences(value, field)` 只按完整 token 边界查找，并返回按源码顺序排列的冻结数组，元素严格为 `{ id, token, start, end, raw, mode }`；`id` 由 store 的本次单调计数器生成，调用方不传 id。value/field 非法时抛稳定 `INVALID_INPUT`，同一 token 重复查询复用 id。`bindContext(id, context)` 只接受 `text`、`image-alt`、`link-label`、`link-url`、`link-title`、`html-attribute`、`raw-html`，返回含 `field/state` 的冻结 occurrence snapshot；未知 id/field 冲突为 `CARRIER_BINDING_ERROR`，非法迁移为 `CARRIER_STATE_INVALID`，并驱动 `issued -> pending-markdown|raw-restored|text-preserved|raw-preserved`。显式 excerpt 的初始状态为 `excerpt-pending`。
- carrier 通过 store 查询/绑定 occurrence，不维护第二套字符串索引、id 生成器或拆分逻辑。Marked 扩展只能消费 store 返回的 occurrence 与 token metadata；parent token 的 `arknights` 字段是已定义形状的 metadata 数组，支持同一父 token 内多枚 occurrence。
...
- 安装的 extension pack 固定包含 `arknights-marker-carrier`（inline start/tokenizer/renderer）、`image`/`link`/`html` 三个字段 adapter renderer 和 `heading` renderer；除 `installMarkedExtension(markedUse)` 外不新增公共导出。`markedUse` 非函数时抛 `Error`，其 `code === 'INVALID_MARKED_USE'` 且 `reason` 不含内部串；成功严格返回 `undefined`。
...
- `hooks.processAllTokens` 的上下文是 Hooks 实例，必须从 `this.options` 读取本次 carrier/options。它在 lexer 完成后、walkTokens 和 parser/renderer 之前按 parent-first 遍历实际 token 树：调用 `store.findOccurrences(value, 'content')` 发现完整 token，调用 `store.bindContext(id, context)` 绑定并迁移状态，再恢复 image alt、link label/URL/title、HTML 属性或 block raw HTML 的字段值。同一 id 已由直接父 token 绑定时，child/heading ancestor 只复制同一冻结 snapshot，不再重复迁移。扩展不得自行用正则拆 token、nonce 或 checksum。
- 每个受影响 child/parent token 通过不可枚举 `token.arknights` 附上冻结数组；数组元素形状固定为 `{ id, token, field, mode, raw, context, state, parent: { type, field }, headingOnly }`。`parent.type` 必须是实际 Marked token 的非空 `token.type` 字符串，`parent.field` 只使用 `text`、`alt`、`href`、`title`、`raw`；非 heading 祖先的 `headingOnly` 为 `null`，heading 祖先为布尔值。同一 image/link/html 父 token 可包含多个 metadata，renderer 必须逐项消费。
- image alt 固定为 `raw-preserve`：`processAllTokens` 从 `image.text` 发现 occurrence、恢复原 marker 文本并迁移为 `raw-preserved`，再由 image renderer 消费 metadata 并委托 `this.parser.renderer.image(token)`；最终只作为 HTML 属性普通文本，不生成 badge/card 或 wrapper。
- link label 固定为 `text-carrier`：`processAllTokens` 从 `link.text` 发现全部 occurrence，把 label token 子树准备为含原始 marker 文本的普通 text token，状态迁移为 `text-preserved`，再由 link renderer 委托 `this.parser.renderer.link(token)`；不生成 wrapper、不调用 handler。
- link URL/title 固定为 `raw-preserve`：`processAllTokens` 分别从 `link.href`/`link.title` 发现全部 occurrence并恢复原字段值，状态迁移为 `raw-preserved`。测试必须让 link URL 的源码实际含有 marker，断言 token 级 href 恢复为原值、最终属性解码后仍为原值、无 token/wrapper/handler；禁止从最终 HTML 猜测。
- HTML 属性中的 marker 固定为 `raw-preserve`：`processAllTokens` 从实际 `html` token 的 `raw`/`text` 发现并恢复全部 occurrence，状态迁移为 `raw-preserved`，再由 html renderer 委托 `this.parser.renderer.html(token)`；不得把 wrapper 写入属性。
- 顶层/块级实际 `token.type === 'html'` 才能绑定 `raw-html`：包括 block-level raw HTML，以及由 lexer 完整保护、因而通常没有待分类 token 的 `pre`/`textarea`/`script`/`style`。普通 inline HTML token 及其间文本保持 Markdown text 语义。block raw occurrence 只能恢复原文，不能调用 parser/registry/handler。
- `processAllTokens` 对每次调用冻结审计快照，并在 `finally` 从当前 parse options 副本 `this.options` 删除本次 `CARRIER_SYMBOL`；删除前已经准备好的 token/parent metadata 不持有 carrier。该操作不修改原始 `data.markdown` temporary options。无 carrier 时直接返回原 token 数组。返回形状固定为同一 `Token[]`；状态/绑定错误抛稳定 `CARRIER_STATE_INVALID`/`CARRIER_BINDING_ERROR`。
- `walkTokens` 的回调只接收一个 token 参数，不能读取本次 options、carrier 全局状态、字段上下文或最终 wrapper；它只能审计 token 上已附着的冻结 metadata、完整 token 边界和 occurrence 状态。审计按唯一 id 集合执行；同一 id 出现在直接 parent、child 与 heading ancestor 的 metadata 数组中是预期的只读投影，不代表重复绑定。parser/renderer 只能在该时序之后运行。
...
- 提供 meta description 使用的通用投影入口。handler 节点在 projection 中替换为 `toPlainText`，周围已渲染结构 HTML 保留给既有 `strip_html` 清理。正常 after 只保存 `content` 与显式 `excerpt` 的 projection；`projectText(data, 'excerpt')` 在无显式 excerpt 时，必须从已保存的 content projection 按字面 `<!-- more -->` 派生 excerpt 前缀视图。没有分隔符时返回与 Hexo 一致的空字符串；该入口不读取 `data.more`、不扫描 marker、不写 `excerpt`/`more`，也不接受 `sourceField: 'more'`。
...
6. 扩展只依据本次 parse 的 options symbol 找到本次 carrier。extension `start`/tokenizer 使用 `this.lexer.options`；`processAllTokens` 使用 `this.options` 完成全部发现/重分类/metadata 附着，并在 finally 删除 symbol；renderer 只使用 `this.parser` 委托；`walkTokens` 只接收 token。没有 carrier 的普通 Hexo/Marked 渲染必须保持 no-op，扩展不得使用全局 current carrier。
...
10. after 完成后保存 content/显式 excerpt 投影并删除本次 render carrier 的临时状态。正文派生的 `excerpt`/`more` 继续由 Hexo priority 10 从已经物化的 content 派生；after 9 不扫描派生字段。`projectText(data, 'excerpt')` 的无显式 excerpt 分支只读取已保存 content projection 并按 `<!-- more -->` 派生，不读取或写回 `data.more`。renderer 拒绝时 after 不会运行；下一次同 data before 负责修复/清理 data bridge 与字段。
...
13. `meta-description.js` 在优先级 20 或更高位置调用 pipeline 的通用投影。派生 excerpt 非空时使用 `projectText(data, 'excerpt')`；projection 为 `null` 时按既有 source 选择回退。真实 Hexo 测试必须证明无显式 excerpt 且含 `<!-- more -->` 的 AI 描述最终只含 handler 纯文本（例如 `PASS 摘要说明`），不含 tooltip、SVG 或状态说明。
...
- 完整 raw-text 保护区与实际 block raw HTML 中的原始 marker 保持原文，不能被转换为 handler DOM；普通 inline HTML 标签及其间文本不享受 raw 保护，仍按 Markdown text 语义处理。受保护代码同样保持原文。
...
- 两次并发 `post.render` 各自持有独立 carrier 和 token store。扩展只在 start/tokenizer/processAllTokens 的对应本次 options 中读取状态，不读取模块级“当前 render”变量。
...
- 测试至少覆盖两个连续 heading-only、成功 AI、失败 marker、PJ inline 不支持、重复空 heading，以及随后正常 heading 的 id 等于无 marker 对照且共享 `_headingId` 不出现空字符串或 `-1` 计数。
...
1. 在 `themes/arknights/scripts/markers/` 下使用 lexer、parser、token、carrier、Marked 扩展、registry、sentinel、pipeline、register 和 AI/PJ handler；Task 4 在现有 `lexer.js` 上增量补齐 `pre`/`textarea`/`script`/`style` 完整 raw-text 保护，不重建 Task 1；同时删除 `raw-html.js`，最终 HTML 标签扫描不再承担来源判断。
...
| Marked 扩展 | `start(src.slice(1))` 索引、custom tokenizer、token hooks、walkTokens、extension renderer、无 bridge 的普通 parse | 扩展 start 返回 tempSrc 零基索引且不 `+1`；processAllTokens 独占发现/重分类/metadata；renderer 只消费 metadata 并委托；无 bridge 时 no-op；每次 renderer reset 后可重装 |
| raw provenance | `x <pre>/<textarea>/<script>/<style> …` 的完整 raw-text 内容、源 block raw `<ol>`/`<blockquote>`/`<div>`/`<table>`；另测普通 inline `<span>`/`<a>`/`<em>` 混合文本 | raw-text 内容不签发 token；仅实际 block `html` token 恢复其 carrier；不生成 handler DOM；inline 标签间文本可物化；不依赖最终标签 allowlist；CRLF 只比较结构语义 |
| Markdown provenance | 普通 Markdown `-`/`1.` 列表、blockquote、GFM table 单元格中的受支持 inline AI marker；image alt、link label、link URL/title、HTML 属性中的 marker，含同一父 token 多 occurrence | 普通 Markdown text carrier 才保持 pending 并正常物化；store API 返回稳定 id/范围，parent `arknights` 为冻结数组；非文本上下文按 raw-preserve/text-carrier 规则恢复原文，禁止 wrapper 和 handler；link URL 源码实际含 marker 并断言原值 |
| heading | marker 在 heading 前、后方、两侧、嵌套强调中、至少两个连续 heading-only、成功/失败 marker、PJ inline 不支持、后续正常 heading | heading-only 保留原 children 并 parseInline，priority 9 仍物化/恢复；无 `id`、无 `.headerlink`；共享 `_headingId` 不出现空键或 `-1`，后续正常 heading id 等于对照 |
...
| Hexo 字段 | 真实 `post.render`、优先级 4/9/10/20+、显式 excerpt、无显式 excerpt 的 `<!-- more -->` 派生、`data.more` 覆盖、meta description | before 只扫描 content/显式 excerpt；pipeline 不读取/扫描/写入 more；priority 10 派生字段；`projectText(data,'excerpt')` 从已保存 content projection 派生；真实 description 只含 `PASS 摘要说明`，无 tooltip/SVG/状态说明 |
...
1. `x <pre>/<textarea>/<script>/<style> …` 完整 raw-text、源 block raw `<ol>`/`<blockquote>`/`<div>`/`<table>` 与 Markdown `-`/`1.`/`>`/GFM table 对照。
2. heading marker 在前、后、两侧、嵌套强调、至少两个连续 heading-only、成功/失败 marker、PJ inline 不支持和后续正常 heading；断言实际 `<hN>`、无 id/headerlink、共享 `_headingId` 无空键/`-1`，并确认 priority 9 对成功与失败 marker 均完成预期物化/恢复。
3. image alt、link label、实际含 marker 的 link URL、link title、HTML 属性及同一父 token 多 occurrence；断言 `parent.arknights` 数组、store 绑定、恢复原值、无 token/wrapper/handler；另测 `<span>`/`<a>`/`<em>` inline 混合文本可物化。
...
6. 显式 excerpt、无显式 excerpt 的 `<!-- more -->` 派生和 `data.more` 覆盖；断言 `projectText(data,'excerpt')` 只从保存的 content projection 派生，meta description 只含 `PASS 摘要说明`，无 tooltip/SVG/状态说明；不测试 standalone more。
7. Task 1–3 探针回归。
...
| 最终 HTML 标签被误当成来源事实 | 源 raw 与 Markdown 生成列表/表格混淆，合法 marker 被错误恢复 | 删除 `raw-html.js` 最终扫描；只接受当前 render 实际 block `html` token 的 provenance；普通 inline HTML 不作为 raw 证据，并做 raw/生成块对照矩阵 |
...
本规格把旧 AI/PJ 分散实现收敛为“原始 Markdown 词法提取、严格语法解析、store-owned occurrence、每次 post.render 私有 carrier、实际 Marked token provenance、冻结 parent metadata、registry 分发、handler 校验渲染、通用纯文本投影”的两阶段架构。最终 HTML 标签不承担 raw 来源判断；lexer 完整保护 raw-text 内容，实际 block `html` token 才恢复 raw carrier，普通 inline HTML 标签间文本保持 Markdown text 语义，非文本字段按 raw-preserve/text-carrier 规则恢复。`processAllTokens` 在 parser 前完成发现/重分类并在 finally 删除 parse options symbol；Marked singleton 只允许短暂持有 parse options，不存在应用级 current carrier。heading-only 保留原 children，让 priority 9 正常物化或恢复，同时不进入 `_headingId` 计数。无显式 excerpt 时，projection 只从保存的 content view 按 `<!-- more -->` 派生，不读取或写入 more。协议仍为一次性硬切换，错误保留原文或安全回退；Alert、Spoiler、Terms 保持独立。
