...
- 文档状态：carrier provenance 架构修订已批准；本轮按第五轮最终裁决收敛 link/image 字段语义、canonical ownership、字段审计阶段、heading 真实 Hexo 门禁、PJ 连续性、可观测性、fallback 与 Hexo 中间渲染边界。本轮仅修订规格、实施计划与 `AGENTS.md`，并写入架构复核报告，不修改运行时代码、测试探针、内容或配置；下述 runtime gate 均未在本轮运行。
...
- `hooks.processAllTokens` 的上下文是 Hooks 实例，必须从 `this.options` 读取本次 carrier/options。它在 lexer 完成后、walkTokens 和 parser/renderer 之前遍历实际 token 树，并独占 content occurrence 发现、context 绑定、字段恢复、重分类、content 审计和 metadata 附着。为解决嵌套 link/image 的 canonical ownership，具体字段按 child-first 决定：先处理更深 child 的 `image.text`/`link.tokens`，严格祖先字段随后只能读取并复制该 occurrence 已绑定的冻结 snapshot；未绑定 occurrence 才由当前具体字段首次 `bindContext`。若同一 id 被两个不兼容具体字段、两条非祖先路径或不同 context 绑定，稳定抛 `CARRIER_BINDING_ERROR`，不得以后一次结果覆盖前一次。字段 dispatcher 的语义优先级固定为：`image.text -> image-alt`；`link.tokens/text -> link-label`；`link.href -> link-url`；`typeof token.title === 'string' -> link-title`；满足 `token.type === 'html' && token.block === true` 的 token -> `raw-html`。`title` 为 `null`、`undefined` 或其它类型时不调用 `findOccurrences`，也不扫描 `link.raw`。
...
- link href/title 固定为 `raw-preserve`：分别从 `link.href` 与 `typeof token.title === 'string'` 的 title 发现全部 occurrence 并恢复原字段值，状态迁移为 `raw-preserved`；`title` 为 `null`/`undefined`/其它类型时跳过，禁止扫描 `link.raw`。测试必须让标准 link URL、autolink、裸 URL 的源码实际含有 marker，并与同段普通 marker 共存；最终断言 `<a href=...>` 解码后的属性、可选 title 属性和 label 文本，不要求最终 HTML 出现未经编码的 Markdown 原文，且无 token/wrapper/handler。
...
- 每次 before 只扫描 `data.content` 与入口处显式存在的字符串 `data.excerpt`。`data.more` 永不扫描、永不由 pipeline 读取或写入；实现探针用会抛异常的 `more` getter/setter 直接调用 pipeline before/after/projectText，证明这三个入口不触碰该属性。Hexo 自身的 excerpt priority 10 会从已物化 `data.content` 派生/覆盖 `data.more`，因此不存在 standalone `more` 输入或 `sourceField: 'more'`。
- `content` 的 occurrence 必须具有 Marked provenance；显式 `excerpt` 不经过 Hexo 的 Markdown parse，store 将其记录为字段独立的 `excerpt-pending`，只能由 after 9 写回自己的字段，不得进入 `processAllTokens` 的 content 审计，也不得借用 content 的 HTML 标签或 provenance。after 9 结束时，每枚 excerpt occurrence 必须恰好为 `consumed` 或已恢复原文的 `failed`。
...
2. `data.more` 不是 marker 输入。Hexo excerpt priority 10 会在 priority 9 之后从已物化 `data.content` 派生并覆盖 `data.more`；pipeline 不扫描、不写入、不投影 `more`。直接调用 pipeline before/after/projectText 的探针使用抛异常 getter/setter spy 覆盖这三个入口；真实 Hexo 字段测试只验证 priority 10 的既有派生，不把 Hexo 自己对 `more` 的读写误算为 pipeline 证据。
...
8. pending wrapper 由 priority 9 验证 token 完整性、store occurrence id/field 绑定、模式和 provenance 次数。raw-restored、text-preserved、raw-preserved、未知 wrapper、跨字段 token 和重复消费都不得调用 handler。嵌套 link/image 的具体 child field 先拥有 occurrence，祖先只复制该冻结 snapshot；不兼容重复所有权抛 `CARRIER_BINDING_ERROR`。
9. `after_post_render` 使用优先级 9。content 与显式 excerpt 各物化一次；普通 Markdown text 的 AI/inline 原位替换，合法连续 PJ 先形成卡片再由 pipeline 包裹 `.projects-grid`。普通文字、非法 PJ、AI、空行或其它 block 打断网格。显式 excerpt 的 `excerpt-pending` 在此阶段单独审计并消费/恢复，不经过 Marked。
...
| carrier bridge | 已有/缺失 `data.markdown`、非枚举 data 属性、可枚举 symbol、`getCarrierFromOptions(options, expectedCarrier)` 身份校验、正常 after、renderer/`onRenderEnd`/`after_render:html` rejection、同 data 重试、重复 before | 实际 Marked options 能读取本次 carrier；正常 after 精确恢复；明确承认 Marked singleton 的短暂 options 强引用；`processAllTokens` finally 后 `marked.defaults.hooks.options`/renderer options 不再含 symbol；任一中间阶段拒绝后同 data before 修复旧字段/descriptor；无应用级 current carrier |
...
| 非文本字段 | 当前 Hexo `image.text` alt 语义、child emphasis/多 occurrence、link label、标准 href、字符串 title、autolink/裸 URL 与同段普通 marker 共存、禁止扫描 `link.raw`、link label 内嵌 image alt 及反向嵌套、HTML 属性 marker | 最终 `<img alt>`、`<a href>`、可选 title 和 label 按 renderer 语义验收，不要求最终 HTML 保留 Markdown 原文；string title 才扫描，null/undefined 跳过；具体 child canonical owner 优先，祖先只复制 snapshot，冲突抛 `CARRIER_BINDING_ERROR`；无 wrapper/handler |
...
| Task 4 字段投影 | 真实 `post.render`、优先级 4/9/10、显式 excerpt、无显式 excerpt 的 `<!-- more -->` 派生、无分隔符回退；直接 pipeline 探针使用抛异常 `more` getter/setter | `processAllTokens` 只审计 content Marked occurrence；显式 excerpt 保持 `excerpt-pending` 并由 after 9 消费/恢复；pipeline before/after/projectText 不读不写 more；Task 4 只断言 `projectText` 投影，不直接断言 `data.description`，后者留到 Task 5 |
...
3. 当前 Hexo `image.text` alt（含 emphasis 源码）、link label、标准/autolink/裸 URL href、字符串 title、null/undefined title、同段普通 marker、link label 内嵌 image alt 与 image alt 内嵌 link label；最终按 `<img alt>`/`<a href,title,label>` 语义断言，不要求 Markdown 原文。
4. store/carrier spy 证明 HTML 属性与四种 raw-text protected 区域没有签发 occurrence/metadata；正常 content 则在实际 Marked token 上断言 metadata descriptor、数组、元素与嵌套 `parent` 深度冻结。
...
7. 显式 excerpt、无显式 excerpt 的 `<!-- more -->` 派生、无分隔符回退完整 content projection；直接 pipeline 探针另用抛异常 `more` getter/setter 证明 before/after/projectText 不读不写。Task 4 不测试 `data.description` 或 meta-description filter。
...
| `data.markdown` bridge 描述符、expected carrier 身份或 symbol 属性配置错误 | 实际 parse 看不到 carrier、旧 options 被误认作本次 carrier，或 data 序列化泄漏内部状态 | `getCarrierFromOptions(options, expectedCarrier)` 做同一引用校验；真实 Hexo 探针断言非枚举 data 属性、可枚举 symbol、正常/失败清理和重复 before |
| 嵌套 link/image 被多个字段重复认领 | 同一 occurrence 被重分类、祖先扁平化或 wrapper 泄漏 | child-first canonical ownership；严格祖先只复制已绑定 snapshot；不兼容冲突抛 `CARRIER_BINDING_ERROR`；覆盖 link→image 与 image→link 两个方向 |
...
本规格把待实施的旧 AI/PJ 分散路径收敛为“原始 Markdown 词法提取、严格语法解析、store-owned occurrence、每次 post.render 私有 carrier、实际 Marked token provenance、深度冻结 parent metadata、registry 分发、handler 校验渲染、通用纯文本投影”的两阶段架构。最终 HTML 标签不承担 raw 来源判断；只有实际 `type==='html' && block===true` 的 token 才恢复 raw carrier。link label 采用 text-carrier；image alt 采用当前 Hexo `image.text`；link href 与字符串 title raw-preserve，最终验收按 renderer DOM 语义。嵌套 link/image 由具体 child field canonical ownership，冲突稳定失败。`processAllTokens` 只审计 content Marked occurrence，explicit excerpt 由 after 9 审计；metadata 深度冻结且 descriptor 固定。renderer/`onRenderEnd`/`after_render:html` 任一拒绝都会跳过 after 9，fallback 以 `carrier.originalField(field)` 为权威。heading-only 与 `_headingId` 门禁必须在真实 `Hexo#post.render` 中验证，纯 `new Marked()` 只覆盖扩展基本行为。当前 runtime 仍是旧 Task 4 baseline，本规格及列出的 gate 尚未实施或运行；Task 4 只验证 projection，meta description 接线属于 Task 5。
