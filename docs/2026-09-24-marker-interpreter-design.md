...
- 文档状态：carrier provenance 架构修订已批准；本轮按第七轮审查裁决闭合原始 angle autolink/GFM 裸 URL 的两阶段 masked-projection 契约、image/link field-aware direct-field ownership、`data.markdown` 自有 `value: undefined` 描述符边界与重复绑定/excerpt 门禁。本轮仅修订规格、实施计划与 `AGENTS.md`，并写入架构复核报告，不修改运行时代码、测试探针、内容或配置；下述 runtime gate 均未在本轮运行。
...
- 若入口已有 `data.markdown`，正常 after 或下一次同 data before 必须按上述反射结果精确恢复，而不是简单删除。carrier 测试必须分别覆盖：own data descriptor 的 `value: undefined`、其它不支持 value、configurable accessor（getter/setter 零调用）、descriptor throwing Proxy、spread throwing Proxy、首次 data define 抛错但已部分写入的 Proxy；逐次断言稳定错误码，并用 `Object.getOwnPropertyDescriptor` 证明原 value 引用与 `writable/configurable/enumerable` 精确恢复。
...
| carrier bridge | 已有/缺失 `data.markdown`、own data descriptor `value: undefined` 与其它不支持 value、非枚举 data 属性、可枚举 symbol、configurable accessor（getter/setter 零调用）、descriptor/spread throwing Proxy、define trap 部分写入后抛错、`getCarrierFromOptions(options, expectedCarrier)` 身份校验、正常 after、renderer/`onRenderEnd`/`after_render:html` rejection、同 data 重试、重复 before | 只有无 own property 才以 `{}` 开始；descriptor 读取失败映射 READ，accessor/不支持 value/configurable=false 映射 DESCRIPTOR，spread 失败为 READ，define 失败为 DEFINE；所有失败路径的原 value 引用与 `writable/configurable/enumerable` 精确恢复，getter/setter 计数为 0；实际 Marked options 能读取本次 carrier；正常 after 精确恢复；`processAllTokens` finally 后 singleton 不再含 symbol；无应用级 current carrier |
