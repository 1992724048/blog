...
- 文档状态：carrier provenance 架构修订已批准；本轮按第六轮审查裁决补齐 Markdown autolink/裸 URL 词法分支与 synthetic link canonical ownership、bridge descriptor 原子回滚、显式 excerpt/抛异常 `more` 的可执行证据。本轮仅修订规格、实施计划与 `AGENTS.md`，并写入架构复核报告，不修改运行时代码、测试探针、内容或配置；下述 runtime gate 均未在本轮运行。
...
- 当当前位置是 `<` 时，Task 4 的增量 lexer 必须先调用与锁定的 Marked 15 inline `autolink` 语法等价的 `scanMarkdownAutolink(source, start)`；命中 `<scheme:...>` 或 `<email>` 的完整 Markdown autolink 时只按普通文本推进，不得进入 `scanRawHtml`/`scanHtmlTag`。因此 `<https://example.com/[#]<AI>{PASS}>` 内的 marker 必须签发恰好一枚 inline occurrence、不得形成 protected segment；GFM `https://example.com/[#]<AI>{PASS}` 不新增专用 raw 分支，也必须由普通扫描签发恰好一枚 inline occurrence。只有 autolink 未命中时，才允许进入 `scanRawHtml`/`scanHtmlTag`。
- autolink 优先分支不得削弱 HTML 属性保护：`<span data-marker="[#]<AI>{PASS}">正文</span>` 仍只把开始标签（含属性）记录为 `protected/raw-html`，其中 marker 数量为 0、occurrence 数量为 0。angle autolink、GFM 裸 URL 各用一个“URL 内唯一 marker + 同一段普通正文”的源码用例验收；最终 `<a>` 断言不能替代源码 occurrence、protected 状态与 carrier context 断言。
...
- carrier 通过 store 查询/绑定 occurrence，不维护第二套字符串索引、id 生成器或拆分逻辑。Marked 扩展只能消费 store 返回的 occurrence 与 token metadata；parent token 的 `arknights` 字段是已定义形状的深度冻结 metadata 数组，支持同一父 token 内多枚 occurrence。canonical ownership 由具体 child field 先绑定，严格祖先只复制该 child 已确定的冻结 snapshot。锁定 Marked 15 的 angle autolink 与 GFM 绝对 URL 裸 token 是一个明确的 synthetic projection：只有当 `token.type === 'link'`、token 没有自有 `title`、`token.text === token.href`，且 `Array.isArray(token.tokens) && token.tokens.length === 1` 的唯一 child 满足 `child.type === 'text' && child.raw === child.text && child.text === token.text` 时，才识别为 synthetic URL link；该关系的唯一 owner 是 `link.href -> link-url`，`link.text` 与合成 child 只复制同一冻结 snapshot，不调用 `findOccurrences`/`bindContext`，也不附独立 metadata。同一 occurrence 被真正不兼容的具体字段或非祖先 sibling 路径重复绑定时稳定抛 `CARRIER_BINDING_ERROR`；synthetic 的 href/text/child 三个投影不构成 sibling 冲突。扩展不得自行拆 token 字符串或复制定位规则。
...
- before/attach 必须先用反射捕获 `data.markdown` 的“自有属性或不存在”以及原 property descriptor；descriptor 获取失败映射为 `CARRIER_BRIDGE_READ`。只有捕获到 data descriptor 后才允许读取 `descriptor.value`；若存在 getter/setter，立即抛 `CARRIER_BRIDGE_DESCRIPTOR`，不得调用 accessor，也不得因 `descriptor.value === undefined` 静默改用 `{}`。入口属性为 `configurable: false` 时同样拒绝，因为临时 bridge 无法在保持原 descriptor 的前提下安装。
...
- 若入口已有 `data.markdown`，正常 after 或下一次同 data before 必须按上述反射结果精确恢复，而不是简单删除。carrier 测试必须分别覆盖 configurable accessor（不调用 getter/setter）、descriptor Proxy、spread Proxy、首次 data define 抛错但已部分写入的 Proxy，并逐次用 `Object.getOwnPropertyDescriptor` 证明 value 引用和三个 flags 精确恢复。
...
- `hooks.processAllTokens` 的上下文是 Hooks 实例，必须从 `this.options` 读取本次 carrier/options。它在 lexer 完成后、walkTokens 和 parser/renderer 之前遍历实际 token 树，并独占 content occurrence 发现、context 绑定、字段恢复、重分类、content 审计和 metadata 附着。为解决嵌套 link/image 的 canonical ownership，具体字段按 child-first 决定：先处理更深 child 的 `image.text`/`link.tokens`，严格祖先字段随后只能读取并复制该 occurrence 已绑定的冻结 snapshot；未绑定 occurrence 才由当前具体字段首次 `bindContext`。Marked 15 synthetic URL link 必须先于通用 link label dispatcher 识别，并跳过 `link.text`/唯一 text child 的独立扫描：只从 `link.href` 绑定一次 `link-url`，把同一 snapshot 投影回 text/child 供 renderer 消费，child 不附 metadata。若同一 id 被真正不兼容的具体字段、两条非祖先路径或不同 context 绑定，稳定抛 `CARRIER_BINDING_ERROR`，不得以后一次结果覆盖前一次。字段 dispatcher 的其余语义优先级固定为：`image.text -> image-alt`；普通 `link.tokens/text -> link-label`；`link.href -> link-url`；`typeof token.title === 'string' -> link-title`；满足 `token.type === 'html' && token.block === true` 的 token -> `raw-html`。`title` 为 `null`、`undefined` 或其它类型时不调用 `findOccurrences`，也不扫描 `link.raw`。
- 每个受影响 child/parent token 通过 `Object.defineProperty` 附加深度冻结数组：descriptor 固定为 `{ value: metadata, enumerable: false, configurable: false, writable: false }`。数组本身、每个元素及嵌套 `parent` 都冻结；元素形状固定为 `{ id, token, field, mode, raw, context, state, parent: { type, field }, headingOnly }`。`parent.type` 必须是实际 Marked token 的非空 `token.type` 字符串，`parent.field` 只使用 `text`、`alt`、`href`、`title`；非 heading 祖先的 `headingOnly` 为 `null`，heading 祖先为布尔值。同一 image/link/html 父 token 可包含多个 metadata，renderer 必须逐项消费。
- image alt 固定为当前 Hexo `image.text` 语义：只从 `image.text` 发现并恢复 occurrence，状态迁移为 `raw-preserved`；若同一 occurrence 已由更深 child（例如 image 内嵌 link label）拥有，image 祖先只复制该 snapshot、按 child 语义恢复自己的字段值，不把 context 改回 `image-alt`，也不重复迁移。`image.tokens` 只用于 canonical ownership 追踪，不改写为另一套规范化 alt，也不要求 child emphasis 被剥离。全部 image-alt occurrence 恢复后，image renderer 只委托 `this.parser.renderer.image(token)`。因此测试以最终 `<img alt>` 的 HTML 属性语义为准；带 Markdown 强调源码时，当前 Hexo renderer 输出其 `image.text`，扩展不得同时声称已委托 renderer 又把强调结构改成另一种 alt。
- link label 固定为 text-carrier：`link.tokens/text` 走同一个 label adapter，发现直接 label occurrence，把需要显示的 carrier child 准备为普通可见 text，状态迁移为 `text-preserved`；嵌套 image/link child 保持其自身 canonical 结构，不被祖先扁平化。link renderer 只委托 `this.parser.renderer.link(token)`，不生成 wrapper、不调用 handler。验收断言最终 `<a>` 的可见 label、href 和 title，而不是要求最终 HTML 保留 Markdown 原文。
...
- 提供后续 meta description 使用的通用投影入口。handler 节点在 projection 中替换为 `toPlainText`，周围已渲染结构 HTML 保留给既有 `strip_html` 清理。正常 after 只保存 `content` 与显式 `excerpt` 的 projection；`projectText(data, 'excerpt')` 在无显式 excerpt 时，必须从已保存的 content projection 按字面 `<!-- more -->` 派生 excerpt 前缀视图，没有分隔符时回退到完整 content projection。该入口不读取 `data.more`、不扫描 marker、不写 `excerpt`/`more`，也不接受 `sourceField: 'more'`。Task 4 验证显式 excerpt 字段的 DOM/终态与 `projectText`，但不读取或断言 `data.description`；`data.description` 和 `meta-description.js` 接线归 Task 5。
...
8. pending wrapper 由 priority 9 验证 token 完整性、store occurrence id/field 绑定、模式和 provenance 次数。raw-restored、text-preserved、raw-preserved、未知 wrapper、跨字段 token 和重复消费都不得调用 handler。嵌套 link/image 的具体 child field 先拥有 occurrence，祖先只复制该冻结 snapshot；synthetic URL link 的 `link.href` 是唯一 owner，`link.text`/合成 child 只复制同一 `link-url` snapshot；只有真正不兼容的具体字段或非祖先 sibling 重复所有权才抛 `CARRIER_BINDING_ERROR`。
...
- bridge 安装的提交点只能是 data descriptor define 成功；在此之前任何 descriptor/spread/symbol define 失败都不得留下临时 options。data define 失败时必须用捕获的原 descriptor/不存在状态回滚，即使 Proxy trap 已先写入再抛错；accessor 必须在读取 `.value` 前拒绝。回滚后的 descriptor 深比较（value 引用及 `writable/configurable/enumerable`）和稳定错误码是 Task 4 必测契约。
...
- 真实 `Hexo#post.render` 测试用 test-only walkTokens/renderer spy 记录实际 heading token 的 `token.tokens`，并对 `parseInline` 入参做引用比较，必须确认两者为同一数组；同时覆盖两个连续 heading-only、成功 AI、失败 marker、PJ inline 不支持、`headerIds: false`，以及随后正常 heading 的 id 等于无 marker 对照且共享 `_headingId` 不出现空字符串或 `-1` 计数。纯 `new Marked()` 不承担这组 Hexo heading 门禁。
...
| carrier bridge | 已有/缺失 `data.markdown`、非枚举 data 属性、可枚举 symbol、configurable accessor（getter/setter 零调用）、descriptor/spread throwing Proxy、define trap 部分写入后抛错、`getCarrierFromOptions(options, expectedCarrier)` 身份校验、正常 after、renderer/`onRenderEnd`/`after_render:html` rejection、同 data 重试、重复 before | descriptor 读取/形状错误分别稳定映射 `CARRIER_BRIDGE_READ`/`CARRIER_BRIDGE_DESCRIPTOR`，spread 失败为 READ，define 失败为 DEFINE；所有失败路径的原 value 引用与 `writable/configurable/enumerable` 精确恢复；实际 Marked options 能读取本次 carrier；正常 after 精确恢复；`processAllTokens` finally 后 singleton 不再含 symbol；无应用级 current carrier |
...
| 非文本字段 | 当前 Hexo `image.text` alt 语义、child emphasis/多 occurrence、link label、标准 href、字符串 title、angle autolink/GFM 裸 URL 的 lexer 非 protected/唯一 occurrence、synthetic href/text/child 单 owner、link label 内嵌 image alt 及反向嵌套、禁止扫描 `link.raw`、HTML 属性 marker | angle autolink 与裸 URL 各自恰有 1 枚 inline marker occurrence、0 个 protected segment，最终 `<a href>` 恢复 marker URL；synthetic link 只有一次 `link-url` bind，parent metadata 为 `field:'href'`、终态 `raw-preserved`，合成 child 无 metadata；普通 link/image 仍按 child-first owner；无 wrapper/handler |
| heading | 真实 `Hexo#post.render` 中 marker 在 heading 前、后方、两侧、嵌套强调中、至少两个连续 heading-only、成功/失败 marker、PJ inline 不支持、`headerIds:false`、后续正常 heading | test-only spy 确认 `parseInline` 收到原 `token.tokens` 同一引用；priority 9 仍物化/恢复；无 `id`/`.headerlink`；共享 `_headingId` 不出现空键或 `-1`，后续正常 heading id 等于对照；纯 `new Marked()` 不作为此门禁 |
...
| Task 4 字段投影 | 真实 `post.render`、优先级 4/9/10、合法/非法显式 excerpt、无显式 excerpt 的 `<!-- more -->` 派生、无分隔符回退；直接 pipeline 探针使用抛异常 `more` getter/setter 与读/写计数器 | `processAllTokens` 只审计 content；合法显式 excerpt 直接断言 `.ai-badge--notreview` DOM 且 occurrence=`consumed`，非法 excerpt 断言具体转义 marker 且 occurrence=`failed`，均无内部串；before/after/projectText 三个入口后 more 计数仍为 0；Task 4 不直接断言 `data.description` |
...
Task 4 的纯 `new Marked()` 测试只覆盖扩展基本行为、无 bridge no-op、token-local descriptor/深度冻结和 store/carrier spy，不承担 Hexo heading、当前 Hexo image alt 或完整 HTML 语义门禁。期望 after 生成 badge/grid 的纯 pipeline 测试仍须使用带 carrier extension 的 `new Marked().parse` 或完整 metadata fixture，禁止手工拼 `<p>`；真实 Hexo 探针负责自动注册、priority、renderer/onRenderEnd/after_render 生命周期和最终 DOM，并至少覆盖：
...
2. heading marker 在前、后、两侧、嵌套强调、至少两个连续 heading-only、成功/失败 marker、PJ inline 不支持、`headerIds:false` 和后续正常 heading；test-only spy 确认 `parseInline` 收到原 children 的同一引用，并断言实际 `<hN>`、无 id/headerlink、共享 `_headingId` 无空键/`-1`，确认 priority 9 对成功与失败 marker 均完成预期物化/恢复。
3. 当前 Hexo `image.text` alt（含 emphasis 源码）、link label、标准 href、字符串 title、null/undefined title、link label 内嵌 image alt 与 image alt 内嵌 link label；另对 `前文 <https://example.com/[#]<AI>{PASS}> 后文` 和 `前文 https://example.com/[#]<AI>{PASS} 后文` 分别断言 lexer 恰一枚 inline marker、零 protected segment、carrier occurrence 恰一枚且 context=`link-url`/state=`raw-preserved`，synthetic parent 只有 `field:'href'` metadata、唯一 child 无 metadata。最终按 `<img alt>`/`<a href,title,label>` 语义断言，不要求 Markdown 原文。
4. store/carrier spy 证明 HTML 属性与四种 raw-text protected 区域没有签发 occurrence/metadata；angle autolink/裸 URL 的唯一 marker 则必须签发 occurrence。对正常 content 在实际 Marked token 上断言 metadata descriptor、数组、元素与嵌套 `parent` 深度冻结；synthetic link 的 child 明确断言没有 `arknights` descriptor。
...
7. 合法/非法显式 excerpt、无显式 excerpt 的 `<!-- more -->` 派生、无分隔符回退完整 content projection。显式 excerpt 用例在 before 后捕获本次 carrier，after 后直接断言最终字段 DOM/恢复文本、occurrence=`consumed`/`failed` 与零内部串；直接 pipeline 探针另用抛异常 `more` getter/setter 及读/写计数证明 before/after/projectText 不读不写。Task 4 不测试 `data.description` 或 meta-description filter。
...
| `data.markdown` bridge 描述符、expected carrier 身份或 symbol 属性配置错误 | accessor 被误读成 `{}`、Proxy 部分写入污染 data、实际 parse 看不到 carrier、旧 options 被误认作本次 carrier | 先拒绝 accessor，再捕获 descriptor/spread/define；用原 descriptor/不存在状态反射回滚并深比较；`getCarrierFromOptions` 做同一引用校验；真实 Hexo 探针断言正常/失败清理和重复 before |
| 嵌套 link/image 或 synthetic URL link 被多个字段重复认领 | 同一 occurrence 被重分类、autolink 的 href/text/child 误报冲突、祖先扁平化或 wrapper 泄漏 | 普通结构 child-first ownership；synthetic URL link 唯一 owner=`link.href/link-url`，text/child 只复制 snapshot；只对真正非祖先 sibling 冲突抛 `CARRIER_BINDING_ERROR`；覆盖 link→image、image→link、angle autolink、GFM 裸 URL |
...
本规格把待实施的旧 AI/PJ 分散路径收敛为“原始 Markdown 词法提取、严格语法解析、store-owned occurrence、每次 post.render 私有 carrier、实际 Marked token provenance、深度冻结 parent metadata、registry 分发、handler 校验渲染、通用纯文本投影”的两阶段架构。最终 HTML 标签不承担 raw 来源判断；只有实际 `type==='html' && block===true` 的 token 才恢复 raw carrier。lexer 在 raw HTML tag 前识别 Markdown autolink，angle autolink/GFM 裸 URL 内的唯一 marker 必须进入 `link-url`，不能被 raw-tag 保护假阳性掩盖。link label 采用 text-carrier；image alt 采用当前 Hexo `image.text`；link href 与字符串 title raw-preserve，synthetic URL link 只有 href owner，text/child 为同一快照投影。bridge 对 accessor/Proxy 失败执行 descriptor 原子回滚。`processAllTokens` 只审计 content，explicit excerpt 由 after 9 审计并以 DOM+`consumed`/`failed` 证明；`data.more` 以抛异常 getter/setter 零计数证明 pipeline 不读写。renderer/`onRenderEnd`/`after_render:html` 任一拒绝都会跳过 after 9，fallback 以 `carrier.originalField(field)` 为权威。heading-only 与 `_headingId` 门禁必须在真实 `Hexo#post.render` 中验证，纯 `new Marked()` 只覆盖扩展基本行为。当前 runtime 仍是旧 Task 4 baseline，本规格及列出的 gate 尚未实施或运行；Task 4 验证显式 excerpt 字段与 projection，但不验证 meta description；后者接线属于 Task 5。
